...
3. **Start agent** — `startAgent(channelName, rtcUid, userUid)` → `POST /api/startAgent`. Backend validates env vars, builds the `CustomLLM` + STT + TTS vendor chain, starts the async session, and returns `agent_id`.
4. **Converse** — user audio flows through Deepgram STT (managed), then Agora cloud calls `POST <CUSTOM_LLM_URL>/chat/completions`; the concierge LLM endpoint runs the persona FSM and streams the reply; MiniMax TTS plays it back. RTM delivers transcript + metrics to the web UI.
